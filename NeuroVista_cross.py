...
class CrossModalBlock(nn.Module):
    ...
    def forward(self, x_eeg, x_img):
        ...
        # 再融合 (EEG 融合自己 + img_after_attn,   Img 融合自己 + eeg_after_attn)
...
class IE():
    ...
    def get_eeg_train_data(self):
        ...
        for sub in self.train_subs:
            ...
            mask_ratio = args.ratio  # 要mask的比例
    ...
    def train(self):
        ...
        for e in range(self.n_epochs):
            ...
            torch.save(self.Enc_eeg.state_dict(), './model_2_cross/Enc_eeg_sub' + format(self.test_sub, '02') + '.pth')
            torch.save(self.Proj_eeg.state_dict(), './model_2_cross/Proj_eeg_sub' + format(self.test_sub, '02') + '.pth')
            torch.save(self.Proj_img.state_dict(), './model_2_cross/Proj_img_sub' + format(self.test_sub, '02') + '.pth')
            torch.save(self.cross_modal_block.state_dict(), './model_2_cross/cross_modal_block' + format(self.test_sub, '02') + '.pth')
        ...
        self.Enc_eeg.load_state_dict(torch.load('./model_2_cross/Enc_eeg_sub'  + format(self.test_sub, '02') + '.pth'), strict=False)
        self.Proj_eeg.load_state_dict(torch.load('./model_2_cross/Proj_eeg_sub' + format(self.test_sub, '02') + '.pth'), strict=False)
        self.Proj_img.load_state_dict(torch.load('./model_2_cross/Proj_img_sub' + format(self.test_sub, '02') + '.pth'), strict=False)
        self.cross_modal_block.load_state_dict(torch.load('./model_2_cross/cross_modal_block' + format(self.test_sub, '02') + '.pth'), strict=False)
...
def main():
    ...
    df_results.to_csv(os.path.join(result_path, f'result__{time_str}_woval_2_cross.csv'), index=False)
    print("Results saved.")
