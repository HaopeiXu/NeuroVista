...
class IE():
    ...
    def train(self):
                ...
                with torch.no_grad():
                    ...
                    for i, (veeg, vimg) in enumerate(self.val_dataloader):
                        ...
                        if vloss <= best_loss_val:
                            best_loss_val = vloss
                            best_epoch = e + 1
                            torch.save(self.Enc_eeg.module.state_dict(), './model-mae/' + model_idx + 'Enc_eeg_cls.pth')
                            torch.save(self.Proj_eeg.module.state_dict(), './model-mae/' + model_idx + 'Proj_eeg_cls.pth')
                            torch.save(self.Proj_img.module.state_dict(), './model-mae/' + model_idx + 'Proj_img_cls.pth')
        ...
        self.Enc_eeg.load_state_dict(torch.load('./model-mae/' + model_idx + 'Enc_eeg_cls.pth'), strict=False)
        self.Proj_eeg.load_state_dict(torch.load('./model-mae/' + model_idx + 'Proj_eeg_cls.pth'), strict=False)
        self.Proj_img.load_state_dict(torch.load('./model-mae/' + model_idx + 'Proj_img_cls.pth'), strict=False)
...
def main():
    ...
    pd_all.to_csv(os.path.join(result_path , f'result__{time_str}_mae.csv'), index=False)
